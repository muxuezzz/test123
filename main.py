...
from src.processors import DetPreprocessDecodeResize, DetPreprocessNormalize, DetPreprocessTranspose, DetInfer, \
    DetPostproces, RecInfer, RecPreprocess, RecPostprocess, ClsProcess, ClsPreProcess
...
def main():
    ...
    if args.use_cls:
        modules = det_modules + cls_modules + rec_modules
    else:
        modules = det_modules + rec_modules

    pipelines = MultiPipelineSystem(args.parallel_num, 32, modules)

    def send(pipelines, image_list):
        for idx, image_path in enumerate(image_list):
            pipelines.send(image_path)

    # 发送几张图片预热
    _send_p = Process(target=send, args=(pipelines, image_list[:5]))
    _send_p.start()
    for _ in range(len(image_list[:5])):
        print("预热")
        try:
            pipelines.get(timeout=args.timeout)
        except:
    ...
    start = time.time()
    send_p = Process(target=send, args=(pipelines, image_list))
    send_p.start()
